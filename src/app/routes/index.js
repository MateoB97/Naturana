...
module.exports = (app) => {
      ...
      if (flagLogin === false) {
         ...
         try {
            connection.query('SELECT * FROM users', (err, result) => {
               ...
               users = result;
               try {
                  res.render('../views/main/ventanas/login/editarClave.ejs', {
                     firstName: firstName,
                     lastName: lastName,
                     users: result
                  });
   ...
   app.post('/editarPass/:id', async (req, res) => {
      ...
      if (flagPass) {
         let hashPass = await bcryptjs.hash(pass, 8);
         console.log(hashPass);
         await connection.query('UPDATE users SET  pass =? WHERE id =?', [hashPass, id], (err) => {
            try {
               if (err) {
                  console.error(`Error de la consulta ${err}`);
               } else {
                  try {
                     res.render('../views/main/ventanas/login/editarClave.ejs', {
                        alert: true,
                        alertTitle: 'Edición de contraseña',
                        alertMessage: "Usuario Editado con éxito",
                        alertIcon: "success",
                        showConfirmButton: false,
                        ruta: "editarPass",
                        users,
         ...
         try {
            res.render('../views/main/ventanas/login/editarClave.ejs', {
               alert: true,
               alertTitle: 'Error',
               alertMessage: "Las contraseñas no coinciden intenta de nuevo",
               alertIcon: "error",
               showConfirmButton: false,
               ruta: "editarPass",
               firstName,
   ...
   app.get('/eliminarUsuario/:id' , async(req , res)=>{
      const id = req.params.id;
      res.render('../views/main/ventanas/login/editarClave.ejs',{
         alert2:true,
         title: '¿Eliminar usuario?',
         text: "En el momento que se elimine el usuario no se puede recuperar",
         icon: 'warning',
   ...
   app.get('/eliminarUsuarioDataBase/:id' , async (req , res)=>{
      let id = req.params.id
      await connection.query('DELETE FROM users WHERE id = ?',id,(err)=>{
         try {
            res.redirect('/editarPass');
         } catch (error) {
